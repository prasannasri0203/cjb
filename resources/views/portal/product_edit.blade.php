...
<input type="hidden" name="status" value="0">
  ...
  <div class="tab card-body">
    ...
    <option value="{{ $list->id }}" {{ (old('supplier_id', $product_data->supplier_id) == $list->id ? "selected":"") }}>{{ $list->name }}</option>
    @endforeach
    </select>
    ...
    <textarea rows = "5" cols = "50" id="product_description"  name="product_description" form="product_add_form">{{ old('product_description', $product_data->product_description) }}</textarea>
    ...
    <label for="product_add_form">Product Variant: <span style="text-transform: capitalize;font-weight: bold;">{{$po->name}}</span></label><br />
    <!-- Default inline-->
    <div class="row">
      ...
      <div class="col-lg-3">
        <div class="custom-control custom-checkbox custom-control-inline">
          <input type="checkbox" class="custom-control-input pov_check" id="pov{{$pov->id}}" value="{{$pov->name}}" name="product_option_value[]" {{ in_array($pov->id, $data_pov) ? ' checked' : '' }}>
          <label class="custom-control-label" for="pov{{$pov->id}}">{{$pov->name}}</label>
        </div>
      </div>
      @endif
      @endforeach
    </div>
    <div class="error" id="product_option_error"></div>
    </div>
    @endforeach
    <div class="form-group">
  ...
  <div class="append-form form_final">
    ...
    <?php
    $pov_data = DB::table('product_option_values')->where('id', $pov->value_id)->first();
    $povname = strtolower($pov_data->name);
    ?>
    <div class="row pov_item_{{$povname}}">
        <div class="col-lg-12">
            <div class="daily-feeds card">
                <div class="card-header">
                    <h3 class="h4">{{strtoupper($pov_data->name)}}</h3>
                </div>
                        ...
                        <div class="feed d-flex justify-content-between"><div class="error" id="form_error2"></div>
                            <div class="col-lg-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="pv_name[{{$povname}}]" value="{{$pov->variant_name}}">
                                </div>
                            </div>
                            <div class="col-lg-4">
                                <div class="form-group">
                                    <label for="product_add_form">Price:  <span class="mandatory_field">*<span></label>
                                    <input type="text" class="form-control" name="pv_price[{{$povname}}]" value="{{$pov->price}}">
                                </div>
                            </div>
                            <div class="col-lg-4">
                                <div class="form-group">
                                    <label for="product_add_form">Quantity:  <span class="mandatory_field">*<span></label>
                                    <input type="text" class="form-control" name="pv_quantity[{{$povname}}]" value="{{$pov->quantites}}">
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="feed d-flex justify-content-between">
                            <div class="col-lg-6">
                                <div class="form-group">
                                    ...
                                    <input id="fileupload_{{$povname}}" class="fileupload" type="file" data-name="{{$povname}}" name="pv_upload[{{$povname}}][m][]" multiple="multiple" accept="image/x-png,image/jpg,image/jpeg"/><div class="error pv_upload_{{$povname}}"></div>
                                </div>
                            </div>
                            <div class="col-lg-6">
                                <div class="form-group">
                                    <label for="product_add_form">Split Image:  <span class="mandatory_field">*<span> <small>Upto 4 images split allowed.</small></label>
                                    <input id="fileupload_sp_{{$povname}}" class="fileupload_sp" type="file" data-name="{{$povname}}" name="pv_upload[{{$povname}}][s][]" accept="image/x-png,image/jpg,image/jpeg"/><div class="error pv_upload_sp{{$povname}}"></div>
                                    <button id="fileupload_sp_button_{{$povname}}" class="fileupload_sp_button" data-type="{{$povname}}" type="button">Split</button><div class="error pv_upload_sp_{{$povname}}" style="float: left;"></div>
                                </div>
                                <input type="hidden" id="fsvalue_{{$povname}}" name="fsvalue_{{$povname}}[]" />
                            </div>
                        </div>
                    </div>
                    <input type="hidden" id="x_{{$povname}}_1" name="x_{{$povname}}_1" />
                    <input type="hidden" id="y_{{$povname}}_1" name="y_{{$povname}}_1" />
                    <input type="hidden" id="w_{{$povname}}_1" name="w_{{$povname}}_1" />
                    <input type="hidden" id="h_{{$povname}}_1" name="h_{{$povname}}_1" />
                    <input type="hidden" id="x_{{$povname}}_2" name="x_{{$povname}}_2" />
                    <input type="hidden" id="y_{{$povname}}_2" name="y_{{$povname}}_2" />
                    <input type="hidden" id="w_{{$povname}}_2" name="w_{{$povname}}_2" />
                    <input type="hidden" id="h_{{$povname}}_2" name="h_{{$povname}}_2" />
                    <input type="hidden" id="x_{{$povname}}_3" name="x_{{$povname}}_3" />
                    <input type="hidden" id="y_{{$povname}}_3" name="y_{{$povname}}_3" />
                    <input type="hidden" id="w_{{$povname}}_3" name="w_{{$povname}}_3" />
                    <input type="hidden" id="h_{{$povname}}_3" name="h_{{$povname}}_3" />
                    <input type="hidden" id="x_{{$povname}}_4" name="x_{{$povname}}_4" />
                    <input type="hidden" id="y_{{$povname}}_4" name="y_{{$povname}}_4" />
                    <input type="hidden" id="w_{{$povname}}_4" name="w_{{$povname}}_4" />
                    <input type="hidden" id="h_{{$povname}}_4" name="h_{{$povname}}_4" />
                    <div class="item clearfix">
                        <div class="feed d-flex justify-content-between">
                            <div class="dvPreview_{{$povname}}">
                              @foreach($pov->product_image as $img)
                              @if($img)
                              <img style="height:100px;width: 100px" src="{{url('portal/img/product/'.$img)}}">
                              @endif
                              @endforeach
                            </div>
                        </div>
                        <div class="feed d-flex justify-content-between">
                            <div class="col-lg-12 dvPreview_sp_{{$povname}}">
                                <div class="col-lg-2 prw_crp_div input-group-prw_1">
                                   <div class="prw_crp">
                                      <img id="cropbox_{{$povname}}_1" class="cropbox"  width="100" height="100"  />
                                      <canvas id="preview_{{$povname}}_1" class="preview" style="width:100px;height:100px;display:none;"></canvas>
                                      <a class="crp_edit" id="crop_{{$povname}}_1" data-toggle="modal" data-target="#myModal" data-backdrop="static" data-keyboard="false">&nbsp;</a>
                                   </div>
                                   <span data-type="{{$povname}}" data-type-value="1" class="crop_del remove_this" onclick="dvpRemove(this);"><i class="fa fa-times-circle"></i></span>
                                </div>
                                <div class="col-lg-2 prw_crp_div input-group-prw_2">
                                   <div class="prw_crp">
                                      <img id="cropbox_{{$povname}}_2" class="cropbox"  width="100" height="100"  />
                                      <canvas id="preview_{{$povname}}_2" class="preview" style="width:100px;height:100px;display:none;"></canvas>
                                      <a class="crp_edit" id="crop_{{$povname}}_2" data-toggle="modal" data-target="#myModal" data-backdrop="static" data-keyboard="false">&nbsp;</a>
                                   </div>
                                   <span data-type="{{$povname}}" data-type-value="2" class="crop_del remove_this" onclick="dvpRemove(this);"><i class="fa fa-times-circle"></i></span>
                                </div>
                                <div class="col-lg-2 prw_crp_div input-group-prw_3">
                                   <div class="prw_crp">
                                      <img id="cropbox_{{$povname}}_3" class="cropbox"  width="100" height="100"  />
                                      <canvas id="preview_{{$povname}}_3" class="preview" style="width:100px;height:100px;display:none;"></canvas>
                                      <a class="crp_edit" id="crop_{{$povname}}_3" data-toggle="modal" data-target="#myModal" data-backdrop="static" data-keyboard="false">&nbsp;</a>
                                   </div>
                                   <span data-type="{{$povname}}" data-type-value="3" class="crop_del remove_this" onclick="dvpRemove(this);"><i class="fa fa-times-circle"></i></span>
                                </div>
                                <div class="col-lg-2 prw_crp_div input-group-prw_4">
                                   <div class="prw_crp">
                                      <img id="cropbox_{{$povname}}_4" class="cropbox"  width="100" height="100"  />
                                      <canvas id="preview_{{$povname}}_4" class="preview" style="width:100px;height:100px;display:none;"></canvas>
                                      <a class="crp_edit" id="crop_{{$povname}}_4" data-toggle="modal" data-target="#myModal" data-backdrop="static" data-keyboard="false">&nbsp;</a>
                                   </div>
                                   <span data-type="{{$povname}}" data-type-value="4" class="crop_del remove_this" onclick="dvpRemove(this);"><i class="fa fa-times-circle"></i></span>
                                </div>
                            </div>
                        </div>
                    </div>
