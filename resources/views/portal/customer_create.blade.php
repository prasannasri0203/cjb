...
<header class="page-header">
	<div class="container-fluid">
		...
		@if($user)<a href="{{ url('admin/order_index') . '?' . http_build_query(['cus_id' => $user->id ]) }}" class="btn btn-success" style="float: right;"> View Orders</a>@endif
	</div>
...
<style type="text/css">
	...
	.date_icon{
		background: url(../../images/date-picker-icon.png) no-repeat 99%;
		/*clear-both:both;*/
	}
		...
		<div class="col-lg-12">
			<div class="card">
				<div class="card-header d-flex align-items-center">
					<h3 class="h4">{{($user) ? 'Edit Customer' : 'Add Customer'}}</h3>
				</div>
				<div class="card-body">
					<p></p>
					@if($user)
					<form method="POST" action="{{route('admin.customer.update')}}" enctype="multipart/form-data"  class="custom-form">
					@else
					<form method="POST" action="{{route('admin.customer.store')}}" enctype="multipart/form-data"  class="custom-form">
					@endif
						@csrf
						...
						<div class="form-group">       
							...
							<input type="text" name="dob" class="form-control datepicker date_icon" id="date"  placeholder="DOB" value="{{($user) ? $user->dob : old('dob')}}" autocomplete="off">
						...
						<div class="form-group">       
							...
							<textarea class="form-control" name="address_2">{{($user) ? $user->address_2 : ''}}</textarea>

							<?php if($errors->has('address_2')){?><span style='color:red'> {{$errors->first('address_2')}}</span><?php }?>
						</div>
						<div class="form-group">       
						...
						<div class="form-group">       
							...
							<a href="{{route('admin.customer_list')}}"> <input type="button" value="Cancel" class="btn btn-danger"> </a>
						</div>
					</form>
				</div>
			</div>
