...
body .ws_order .ordfilt h3.mob_filt {
    top: -5px !important;
...
@section('content')
    ...
    <div class="container tshirt_sec wishtshirt wishlist-tr">
        ...
        @if($data->count() ==0)
            <h2 align="center">No Product</h2>
        @else
            @foreach($data as $value)
                @php
                
                $image = App\MerchandiseProductImages::where('merch_product_id',$value->merch_product_id)->first();
                $proct_name = App\MerchandiseProduct::where('id',$value->merch_product_id)->first();
                @endphp
                @if($proct_name != null)
                 <div class="col-md-4 col-sm-6 ipad_lee">
                   <div class="tshirt_cart add_bsk_btn">
                    ...
                    <a class="wishlisticon">
                        <span>
                            <i onclick="return wishlistadd({{$value->merch_product_id}})" class="fa fa-heart heartin"></i>
                        </span>
                    </a>
                    <div class="price_cart">
                        <div class="row">
                            <div class="col-xs-7">
                                <p>{{ $proct_name->name_creation}}</p>
                                <h4 class="theme">{{currency($proct_name->product_price, 'GBP', session()->get('my_currency'))}}</h4>
                            </div>
                            <div class="col-xs-5">
                                <a href="javascript:void(0);" data_id="{!! $value->merch_product_id !!}" class="add_bsk" class="add_bsk">
...
@section('footer_script')
    ...
    success: function(status){
      console.log(status);
      if(status.return_status == 0)
      {
        alert("Out of stock");
        //this_button_icon.removeClass("fa fa-heart-o");
        iziToast.success({ title: 'Success', message: 'Out of stock', position: 'topRight', });

      }
      iziToast.success({ title: 'Success', message: 'Removed from wishlist', position: 'topRight', });
    ...
    <script>
        ...
        $.ajax({
            ...
            data: {
                    product_id: merchandise_id,  _token : '{{ csrf_token() }}',
                },
            success: function (data) {
                  if(data.status == 'success')
                  {
                    $('.cart_count').html(data.item_count);
                    $('.cart_count_a').attr('href',"{{ url('cart') }}")
