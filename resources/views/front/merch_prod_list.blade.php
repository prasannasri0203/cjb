...
@section('title', 'Merchandize Product')
...
@section('header_script')
    ...
    <style type="text/css">
        ...
        {
            .col-md-12.col-sm-12.pagination ,
             .col-md-12.col-sm-12 .pagination {
    margin: 20px auto !important;
}
.col-md-12.col-sm-12 .pagination li {
    margin: 0px 2px !important;
}
.col-md-12.col-sm-12 .pagination li a {
...
.pag1 ul li.disabled span.page-link:first-child {
    margin-right: 10px;
}
    </style>
    <script>
...
<div class="banner wishbanner">
        ...
        <div id="mymerchand" class="container filtersec firstfilter ws_order">
            <div class="row">
                ...
                <div class="col-sm-8 col-xs-5 filerby pull-right ordfilt">
                    ...
                    <h3 class="cate">Category:
                             <span>{{ request()->query('category_name')[0] != null ? request()->query('category_name')[0]:'All' }}
                            <img src="{!! asset('images/filtdrop.png')  !!}"/>
                        </span>
                    </h3>
                    <h3 class="size">Name:
                        <span>{{ request()->query('artist_name') != null ? request()->query('artist_name'):'All' }}
                        <img src="{!! asset('images/filtdrop.png')  !!}"/>
                    </span>
                    </h3>
            ...
            <div id="seller_common_wrapp" class="row mob_dro mob_ipad_dro">
                ...
                <a href="{{url('search')}}" class="mobclearall">Clear all
                    <img src="{!! asset('images/refresh.png')  !!}" />
                </a>
                <button class="btn applybtn">APPLY</button>
            </div>
        </div>
        <form method="POST" action="{{ url('/merchandise_product_list') }}" id="merch_filter_form">
        @csrf
        <!-- {{ request()->get('search') }} -->
        <!-- <input type="hidden" name="search" value="{{ request()->get('search') != null ? request()->get('search'):'' }}"> -->
        <!-- <input type="hidden" name="drone" value="{{ request()->get('drone') != null ? request()->get('drone'):'' }}"> -->
        <div class="row dro">
            <div class="col-sm-12 cat_drop drop_filter">
                <i class="fa fa-times-circle closedrop"></i>
                <ul class="filter_drop" id="cat_fil">

                @foreach($get_cat as $product_names)
                            <li>
                                <input type="checkbox" name="category_name[]" class="form-control" id="{{$product_names->category_name}}" value="{{$product_names->category_name}}">
                                <label for="{{$product_names->category_name}}"> {{$product_names->category_name}}</label>
                            </li>
                @endforeach
                </ul>
                <a href="{{url('search')}}" class="clearall" >Clear all
                    <img src="{{ asset('images/refresh.png') }}" />
                </a>
            </div>
        </div>
    ...
    <div class="container tshirt_sec wishtshirt">
        ...
        @foreach($merch_product as $product_list)
                ...
                <div class="tshirt_cart add_bsk_btn">
                    ...
                    <div class="price_cart">
                        <div class="row">
                             @if($x != 1) 
                            <div class="col-xs-7">
                                <p>{!! ucfirst($product_list->name_creation)!!}</p>
                                <h4>{!! currency($product_list->product_price, 'GBP', session()->get('my_currency')) !!}</h4>
                            </div>
                            ...
                            @else
                                ...
                                <div class="col-xs-7">
                                    <p>{!! ucfirst($product_list->product_name)!!}</p>
                                    <h4>{!! currency($product_list->product_price, 'GBP', session()->get('my_currency')) !!}</h4>
                                </div>
        ...
        <div class="col-md-12 col-sm-12 pagination pagination_wr">
            <ul class="pagination pagination pag1">
                    <!-- {{ $merch_product->links() }} -->
                    {{ $merch_product->appends(request()->input())->links()}}
            </ul>
        </div>
        @endif
    </div>
