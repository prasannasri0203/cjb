...
class CartController extends Controller
{
    ...
    public function cardPageView(){
        ...
        if (Auth::check()) {
            if((isset(Auth::user()->type) && Auth::user()->type == 2 && (Auth::user()->type == 1))){
    ...
    public function cardPageViewGuest(){
        ...
        $print_cost = 0;
                ...
                if($cartData != null){
                    ...
                    $cartItemDetail = DB::select("SELECT * FROM  cart_management as cm  
                                          ...
                                          AND cim.cart_id = $cartData->id
                                          AND cim.deleted_at is null");
                    $cartCount=session()->put('cartCount', count($cartItemDetail));
                    $cartCount=count($cartItemDetail);
                    $cart_id = $cartData->id;
                    $print_cost = $cartData->print_price;
                }
        ...
        return view('front/cart/cart_page_view',compact('cartCount','delivery','packing','cartList','delivery_id','packing_id','print_type','print_price','table_value','cart_id','print_cost','additional_charge'));
    }
            ...
            if (Auth::check()) {
                ...
                if($newOrderId){
                    ...
                    foreach ($cartData->cartItems as $key => $value) {
                        ...
                        // print_r($product_details->ProductDetails->supplierName);exit; 

                        // lowest order supplier assign product 
