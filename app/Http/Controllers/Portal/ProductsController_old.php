...
class ProductsController extends Controller
{
    ...
    public function product_list(Request $request)
    {
            ...
            foreach ($data as $key => $list){
                $supp = Supplier::where('id', $list->supplier_id)->first();
    ...
    public function productOptions(Request $request)
    {
            ...
            $options_varaints = DB::table('product_option_values')->whereIn('product_option_id', $options)->orderBy('product_option_id')->orderBy('name')->get();

        // return $options_varaints; 
        return response()->json([
            'status' => true,
            'products' => $options_varaints
        ], 200);
    }
    ...
    public function product_save(Request $request)
    {
      ...
      // dd($request->all());
            ...
            {
                $req_pov[] = @$request->input('product_option_value_'.$po_list->id.'_1');
            }
            // dd($req_pov);
            ...
            {
                $x = 0;
                foreach ($request->product_option_value_1_1 as $pokey => $povalue)
                {
                    $i = 2;
                    $attr_key = $pokey;

                    $prd_attri = array(@$req_pov[0][$attr_key], @$req_pov[1][$attr_key]);
                    ...
                    if($pvariant->id)
                    {
                                ...
                                foreach($promotion_images[1] as $k => $v){
                                    ...
                                    $product_variant_images = array('id' => Str::random(32),
                                  ...
                                  // echo "<pre>";echo $pvariant->id; print_r($product_variant_images);
    ...
    public function product_edit($id)
    {
        ...
        $data_pov = Product_variant::where('product_id', $id)->pluck('value_id')->toArray();
...
//dd($size_val);
    ...
    //    dd($request->all());
        ...
        else
        {
            ...
            if(@$promotion_images[strtolower(@$request->product_option_value[0])][0]) {
                ...
                $prd_main = array('front_view' => $promotion_images[strtolower($request->product_option_value[0])][0],
            ...
            if($product->id)
            {
                ...
                foreach ($request->pv_name as $pokey => $povalue)
                {
                    ...
                    $productVar = Product_variant::where('product_id', $product->id)->where('option_id', $pov_db->product_option_id)->where('value_id', $pov_db->id)->first();
                    if($productVar == null) {
                        if($pov_db) {
                            $pv_images = array('front_view' => @$promotion_images[$pokey][0],
                                'back_view' => @$promotion_images[$pokey][1],
                                'thrid_view' => @$promotion_images[$pokey][2],
                                'fourth_view' => @$promotion_images[$pokey][3],
                            );

                            $product_variants[] = array('product_id' => $product->id,
                ...
                if($product_variants)
                {
                    Product_variant::where('product_id', $product->id)->whereNotIn('value_id', $request->product_option_value)->delete();
    ...
    public function pvUpdate(Request $request)
    {
        ...
        // dd($request->all());
        $delete_image = DB::table('product_images')->where('product_variant_id',$request->product_variant_id )->delete();

        $pvd = Product_variant::where('id', $request->product_variant_id)->first();
        ...
        if(!empty($promotion_images[1])) {
            ...
            $pv_images = array($promotion_images[1][0],
        ...
        return redirect()->route('admin.product_list')->with('success','Product variant updated successfully');
    ...
    {
        $id = $request->userId;
        DB::table('product_images')->where('product_variant_id', $id)->delete();
        ...
        if ($request->ajax()) {
            ...
            if($request->status_filter == 3)
            {
                $data = DB::table('reviews')
                        ->join('merchandise_products', 'reviews.product_id', '=', 'merchandise_products.id')
    ...
    public function review_update_data(Request $request)
    {
        ...
        Review::where('id',$request->review_id)->update(['review_status'=>$request->review_filter]);   
                    $response = array(
                        'status' => true,
                    );
                    return response()->json($response);   
...
public function print_type_list_post(Request $request)
    {
        ...
        {
            $rules = array('print_type_name' => 'required|unique:print_types,print_type_name,'.$request->table_id);
        }
        else
        {
            $rules = array('print_type_name' => 'required|unique:print_types');
        }

        $rules = array('print_type_name' => 'required|unique:print_types,print_type_name,'.$request->table_id);
        // $rules = array('print_type_name' => 'required|unique:print_types,'.$request->table_id);
        $validator = Validator::make(Input::all(), $rules);
        ...
        {
            Print_types::where('id',$request->table_id)->update(['print_type_name'=>$request->print_type_name]);
    ...
    {
        $rules = array('image_name' => ['required'],'type_name' => ['required'],'banner_image' => ['required']);
        $validator = Validator::make(Input::all(), $rules);
        if ($validator->fails())
        {
            $response = array(
               ...
               foreach ($request->pv_name as $pokey => $povalue)
                {
                    $i = 2;
                    $attr_key = $pokey -1 ;
                    $prd_attri = array($request->product_option_value_1_1[0], $request->product_option_value_2_1[0]);
                    //dd($prd_attri);

                    $pv_images = array(@$promotion_images[$pokey][0],
