...
class ProductController extends Controller
{
    ...
    public function detail_view($id)
    {
        ...
        // dd($productdetails->category_id,$productdetails->id,Auth::user()->id);
        ...
        // ->where('m.status','1')
        // // ->where('p.id','=',$productdetails->id)
        // ->where('m.artist_id', Auth::user()->id)
        // ->where('m.deleted_at',null)
        // ->groupBy('m.merchandise_master_id')
        // ->take(3)
        // ->get();
        ...
        // $product_varient = Product_variant::with('ProductImages')->where('product_id',$merchproduct->product_id)->get();

        // $product_varient = DB:: select("SELECT * FROM product_variants as PV INNER JOIN merchandise_products as MP 
        //        ON MP.product_variant_id =PV.id 
        //     WHERE MP.merchandise_master_id = ".$merchproduct->merchandise_master_id);
    ...
    public function merchCategoryDescriptionPage()
    {
        $category = Categories::whereNull('deleted_at')->where('parent_id',0)->get();
        $cat_list = ArtistCategory::get();
        return view('front/merch_category',compact('category','cat_list'));
    ...
    public function merchDescriptionPage()
    {
            return view('front/merchandise_detail_page');
    }
...
public function merch_subcat_ajax(Request $request)
    ...
    public function search()
    {
        ...
        //     $cat_list = ArtistCategory::get();
        //     return view('front/merch_sub_category',compact('subcat','cat_list'));
        // }
        // else{
            ...
            //         ->join('product_images', 'product_images.product_id', '=', 'products.id')
            //         ->where('products.category_id',$id)
            //         ->where('products.sub_category_id',$productId)
            //         ->when(!empty($search_query ), function($query) use ($search_query  ) {
            //         $query->where('products.product_name', 'LIKE','%'.$search_query.'%');
            //         })
            //         ->groupBy('products.id')
        ...
        $subCategoryId = '';
            ...
            // $productsDetails = DB::select('SELECT * FROM products WHERE product_name LIKE "%'.$search_query.'%" ');
            // $category_id=$productsDetails[0]->category_id;
            // $product_id=$productsDetails[0]->sub_category_id;
            
            // return redirect('/merch_sub_category/'.$category_id.'/'.$product_id);

        }else{      
        
           //  if($Products_check_box){
           //      $x = 1;
           //      // $admin_product = Product::whereLike('product_name',$search_query)->get();
           //      // $merch_product = MerchandiseProductImages::with(['merch_image','ProductDetails.ProductCategory']);
           //      // $merch_product = $merch_product->whereHas('merch_image',function($query) use($admin_product){
           //      //     $query->where('product_id',$admin_product->id);
           //      // })->paginate(6);

           //      $merch_product = DB::table('products')
           //      ->when(!empty($search_query ), function($query) use ($search_query  ) {
           //          $query->where('products.product_name', 'LIKE','%'.$search_query.'%');
           //      })
           //      ->join('merchandise_products', 'merchandise_products.product_id', '=', 'products.id')
           //      ->join('categories', 'categories.id', '=', 'products.category_id')
               //      ->join('merchandise_product_images', 'merchandise_product_images.merch_product_id', '=', 'products.id')
           ...
           //  {   
                ...
                // $merch_product = MerchandiseProductImages::whereHas('merch_image.ProductDetails')
                //     ->whereHas('merch_image.artistDetails')
                //     ->with(['merch_image','ProductDetails.ProductCategory'])
                //     ->paginate(6);
                //     dd($merch_product);
            // }
        }
   ...
   // dd($merch_product);
        // $productList =  Auth::user()->id;
        // $merch_product = DB::table('merchandise_products AS a')
        // ->leftJoin('artist_category AS c', 'a.artist_category_id', '=', 'c.id')
        // ->leftJoin('merchandise_product_images AS i', 'a.id', '=', 'i.merch_product_id')
        // // ->select('a.artist_category_id','a.id','i.image','c.category_name','a.merchandise_product_name','a.product_price')
        // // ->where('c.user_id',Auth::user()->id)
        // ->where('a.status','1')
        // ->where('a.deleted_at',null)
        // // ->where('a.artist_id', Auth::user()->id)
        // ->groupBy('a.merchandise_master_id')
        // ->orderBy('c.sort_order', 'ASC')     
        // ->paginate(6);
        //     $merch_product = MerchandiseProductImages::with(['merch_image','ProductDetails.ProductCategory']);
        //     if($request->merchandise_product_name){
        //         $merch_product = $merch_product->whereHas('ProductDetails.ProductCategory', function($q) use($request){
        //             $q->where('category_name',$request->merchandise_product_name);
        //         });
        //     }
        //     if($request->artist_id){
        //         $merch_product = $merch_product->whereHas('merch_image',function($q) use($request){
        ...
        // $merch_product = $merch_product->paginate(6);
            // dd($merch_product);
        $get_cat = Categories::where('parent_id',0)->whereNull('deleted_at')->get();
        $user = User::where('type',1)->get();
        $product_name = MerchandiseProduct::where('deleted_at',null)->get();
